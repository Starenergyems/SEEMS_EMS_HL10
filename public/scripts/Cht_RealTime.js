...
async function afterLoadDCM() {
  asdfg = "哈哈阿哈哈";
  console.log("DCM 加載了~ " + asdfg);

  const defaultBut_navBar = document.querySelector("#nB_Chart");
  defaultBut_navBar.classList.add("default_nB");

  let getData = await update_RealTimeChart_Duration(60000);
  console.log(getData);

  update_xMin_xMax(getData.xMin, getData.xMax);
}
...
function update_xMin_xMax(StartTime, EndTime) {
  ...
  cht_RealTime.options.scales.x.min = StartTime;
  cht_RealTime.options.scales.x.max = EndTime;
  ...
  // return { xMin: cht_RealTime.options.scales.x.min, xMax: cht_RealTime.options.scales.x.max };
}
...
btn_Set.addEventListener("click", setTimeRange);
async function setTimeRange() {
  if (Number.isNaN(Number(duration.value)) || Number(duration.value) <= 0) {
    ...
    let getData = await update_RealTimeChart_Duration(v_Duration);
    console.log(getData);
    ...
    // dataPost("http://localhost:3000/chart/realtime", duration.value, unitDura_Select.value); //port要改
  }
}

async function update_RealTimeChart_Duration(Duration) {
  try {
    console.log("嘗試向後端發出請求");
    const response = await fetch("/update_RealTimeChart_Duration", {
      method: "post",
      headers: {
...
let query_1stTime = true;

async function updateData() {
  let getData = await query_RealTimeChart_Data(query_1stTime);
  console.log(query_1stTime);
  console.log(getData);
  query_1stTime = false;

  for (let i = 0; i < cht_RealTime.data.datasets.length; i++) {
    cht_RealTime.data.datasets[i].data = cht_RealTime.data.datasets[i].data.concat(getData[cht_RealTime.data.datasets[i].label]);
    cht_RealTime.data.datasets[i].data = cht_RealTime.data.datasets[i].data.slice(-10000);
  }

  update_xMin_xMax(getData.xMin, getData.xMax);
}

async function query_RealTimeChart_Data(firstQuery) {
  try {
    console.log("嘗試向後端發出請求");
    const response = await fetch("/query_RealTimeChart_Data", {
      method: "post",
      headers: {
        "Content-Type": "application/json",
      },
...
//   let xMinMax = update_xMin_xMax(searchStartT.textContent, searchEndT.textContent);

//   // let n = cht_RealTime.data.datasets.length;
//   // for (let i = 0; i < n; i++) {
//   //   let newElement_data = { x: "", y: null };
//   //   // { x: '2023-11-29 21:24:57.456', y: 60.35 }
//   //   newElement_data.x = xMinMax.xMax;
...
function updateRegDateSetting() {
  ...
  for (j = 0; j < m; j++) {
    for (i = 0; i < n; i++) {
      if (cht_RealTime.data.datasets[i].label === qSelectAll_rDName[j].textContent) {
        cht_RealTime.data.datasets[i].borderColor = qSelectAll_rDColor[j].value;
        cht_RealTime.data.datasets[i].backgroundColor = qSelectAll_rDColor[j].value + "90";

        cht_RealTime.options.scales[qSelectAll_yAxisID[j].textContent].display = Math.round(Number(qSelectAll_yDisplay[j].value));
        cht_RealTime.options.scales[qSelectAll_yAxisID[j].textContent].position = qSelectAll_yPosition[j].value;
        cht_RealTime.options.scales[qSelectAll_yAxisID[j].textContent].title.text = qSelectAll_rDName[j].textContent + " (" + qSelectAll_rDUnit[j].value + ")";
        cht_RealTime.options.scales[qSelectAll_yAxisID[j].textContent].min = Math.round(Number(qSelectAll_rDMin[j].value) * 1000) / 1000;
        cht_RealTime.options.scales[qSelectAll_yAxisID[j].textContent].max = Math.round(Number(qSelectAll_rDMax[j].value) * 1000) / 1000;
