...
const sysMode_MT = { 0: "不動作", 1: "全部自動", 2: "全部手動" };
const statusAllPCS_MT = { 0: "不動作", 1: "全部投入", 2: "全部切離" };
const statusAllBMS_MT = { 0: "不動作", 1: "全部投入", 2: "全部切離" };

let cmd_sysMode = 0;
let cmd_statusAllPCS = 0;
let cmd_statusAllBMS = 0;

async function query_SysCtrl_KeyValuePairs(req) {
  ...
  try {
    ...
    SysCtrl_KeyValuePairs = {
      ...
      sysMode: sysMode_MT[cmd_sysMode],
      P_Project: Scale_Data(GC10Data.System[400001], 0.01, 2),
      P_LoadShift: Scale_Data(GC10Data.System[400002], 0.001, 3),
      statusAllPCS: statusAllPCS_MT[cmd_statusAllPCS],
      statusAllBMS: statusAllBMS_MT[cmd_statusAllBMS],
...
router.post("/record_specific_dataName_to_be_set", async (req, res) => {
  try {
    specific_dataName = req.body.dataName;

    const response = "dataName已記錄~";
...
let lastParas_specific_cmd = [];

router.post("/set_specific_dataStatus", async (req, res) => {
  try {
    const setCmd_raw = req.body.setValue;
    ...
    if (specific_dataName === "statusAllPCS") {
      if (setCmd_raw === "1") {
        newdwctrlData.lc1.W407007 = 1;
        newdwctrlData.lc1.W407012 = 85;

        newdwctrlData.lc2.W407007 = 1;
        newdwctrlData.lc2.W407012 = 85;

        newdwctrlData.lc3.W407007 = 1;
        newdwctrlData.lc3.W407012 = 85;

        newdwctrlData.lc4.W407007 = 1;
        newdwctrlData.lc4.W407012 = 85;

        cmd_statusAllPCS = 1;
      } else {
        newdwctrlData.lc1.W407007 = 2;
        newdwctrlData.lc2.W407007 = 2;
        newdwctrlData.lc3.W407007 = 2;
        newdwctrlData.lc4.W407007 = 2;

        cmd_statusAllPCS = 2;
      }
    } else if (specific_dataName === "statusAllBMS") {
      if (setCmd_raw === "1") {
        newdwctrlData.lc1.W407008 = 2;
        newdwctrlData.lc2.W407008 = 2;
        newdwctrlData.lc3.W407008 = 2;
        newdwctrlData.lc4.W407008 = 2;

        cmd_statusAllBMS = 1;
      } else {
        newdwctrlData.lc1.W407008 = 1;
        newdwctrlData.lc2.W407008 = 1;
        newdwctrlData.lc3.W407008 = 1;
        newdwctrlData.lc4.W407008 = 1;

        cmd_statusAllBMS = 2;
      }
    } else {
      let setValue;
      let setValue_old = Convert_UInt_to_BitString(newdwctrlData.System.W400076, 16).bitString;
      console.log(setValue_old);

      if (setCmd_raw === "1") {
        setValue_old = setValue_old.slice(0, 15 - 0) + "1" + setValue_old.slice(15 - 0 + 1);

        cmd_sysMode = 1;
      } else {
        setValue_old = setValue_old.slice(0, 15 - 15) + "1" + setValue_old.slice(15 - 15 + 1);
      ...
      newdwctrlData.System.W400076 = setValue;
    }
    ...
    const data_MT = {
      sysMode: { dicName: `System`, dataID: "W400076", category: "系統模式15", device: `GC`, log_dataName: `子系統運作模式`, 1: `全部自動`, 2: `全部手動` },
      statusAllPCS: { dicName: `lc1~lc4`, dataID: "W407007", category: "設備控制03", device: `LC1~LC4`, log_dataName: `PCS狀態`, 1: `全部投入`, 2: `全部切離` },
      statusAllBMS: { dicName: `lc1~lc4`, dataID: "W407008", category: "設備控制04", device: `LC1~LC4`, log_dataName: `電池狀態`, 1: `全部投入`, 2: `全部切離` }
    };

    let data_AfM = data_MT[specific_dataName];
    ...
    const doc = {
      ...
      content: `將${data_AfM.log_dataName}設為${data_AfM[setCmd_raw]}`
    };
    console.log(doc);
    ...
    lastParas_specific_cmd.push({ dataName: specific_dataName, setCmd: setCmd_raw });
    setTimeout(function () { reset_specific_dSS_cmd(); }, 4500);
...
async function reset_specific_dSS_cmd() {
  console.log("執行reset_specific_dSS_cmd!!!");
  const oldParas_specific_cmd = lastParas_specific_cmd.shift();
  const dataName = oldParas_specific_cmd.dataName;
  const setCmd = oldParas_specific_cmd.setCmd;
  console.log(dataName);
  console.log(setCmd);

  if (dataName === "statusAllPCS") {

    cmd_statusAllPCS = 0;

  } else if (dataName === "statusAllBMS") {
    ...
    if (setCmd === "1") {
      setValue_old = setValue_old.slice(0, 15 - 0) + "0" + setValue_old.slice(15 - 0 + 1);
    } else {
      setValue_old = setValue_old.slice(0, 15 - 15) + "0" + setValue_old.slice(15 - 15 + 1);
    }
